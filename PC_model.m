function [v,avg_mem_potential,network_freq,phi2] = PC_model(N,tfinal,dt,gsyn,Iapp_meanIN,Iapp_stdIN)
% 
...
a  =  0.03*ones(N,1);         % parameter a describes time scale of recovery
b  = -2*ones(N,1);      	% parameter b describes sensitivity of u to v
c  =  -50*ones(N,1);         %+15*re.^2;         % parameter c describes after-spike reset value of v
d  =   100*ones(N,1);          %0.1*ones(N,1);   %8-6*re.^2;           	 % parameter d describes after-spike reset value of u
%S  =  0.5*rand(N,N);  
Cm =   100*ones(N,1);
k  =  0.7*ones(N,1);
vr = -60*ones(N,1);  % -64 from Hu et al 2011
vt = -40;  %*ones(N,1);  % threshold set to -35 mV from Hu et al 2011
vpeak = 35;
alpha = (1/0.27);
beta  = (1/1.8);
...
tau_s = 1/(alpha+beta);
esyn  = -85;   %*ones(N,1);   
k_low = k; %1.4;  % my guess for SOM cells
k_high = k; %1.4;


% ----- Gap junction parameters -------------------------------------------------------
g_gap = 0; %2.1 +- 2.3 nS according to Gibson 1999 recorded in rat ;  % in nS  
% -------------------------------------------------------------------------------------
...
%v = 10*rand(N,1)-64*ones(N,1); % randomized initial values   
v = vr;       % Initial values of v, all -65 mV
...
for step=1:timesteps           
  ...
  uplot(step)           = u(1);
	...
	fired = find(v(:,step)>=vpeak);    			 % indices of cells that spiked. I'm using 11 mV because I measured spike height in Hu et al 2011.  
  fired_cells_indx(:,step) = v(:,step)>=vpeak;            % logical array of whether cell fire 
