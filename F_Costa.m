...
%
 ...
 % inline function for heaviside. faster than calling a separate function in separate file

heavi = inline('(x>0) +0.5*(x==0)','x');
...
F(1) = (F_W_max *0.5*(1 + tanh((g_GW * X(5) + g_AW * X(6) - beta_W)/alpha_W)) - X(1))/tau_W;
F(2) = (F_N_max *0.5*(1 + tanh((g_EN * X(4) + kappa*X(7))/alpha_N))           - X(2))/tau_N;
F(3) = (F_R_max *0.5*(1 + tanh((g_ER * X(4) + g_GR *X(5)+ g_AR * X(6) - beta_R)/alpha_R))-X(3))/tau_R;
F(4) = (tanh(X(1)/gamma_E) - X(4))/tau_E;
F(5) = (tanh(X(2)/gamma_G) - X(5))/tau_G;
F(6) = (tanh(X(3)/gamma_A) - X(6))/tau_A;
F(7) = ((H_max-X(7))/tau_hw)*heavi(X(1)-theta_W) - (X(7)/tau_hs)*heavi(theta_W-X(1));

% cortex model
w_KNa = 0.37/(1+(38.7/X(18))^3.5);
I_KNa = X(19) * w_KNa * (X(8) - E_K);
F(8)  = -(g_L * (X(8)-E_L_p) + g_AMPA * X(10) * (X(8) - E_AMPA) + g_GABA * X(12) * (X(8) - E_GABA) )/tau_p - (1/C_m) * I_KNa;
F(9)  = -(g_L * (X(9)-E_L_i) + g_AMPA * X(11) * (X(9) - E_AMPA) + g_GABA * X(13) * (X(9) - E_GABA) )/tau_i;
F(10) = X(14);
F(11) = X(15);
F(12) = X(16);
F(13) = X(17);
get_Qp = Qp_max / (1 + exp(-C1 * (X(8) - theta_p) / X(20)));
get_Qi = Qi_max / (1 + exp(-C1 * (X(9) - theta_i) / sigma_i));
Na_pump = R_pump*(X(18)^3/(X(18)^3+3375) - Na_eq^3/(Na_eq^3+3375));
...
F(14) = gamma_e^2 * (N_pp * get_Qp - X(10)) - 2 * gamma_e * X(14);  %  dphi multiplies randn because randn gives SD of 1
F(15) = gamma_e^2 * (N_ip * get_Qp - X(11)) - 2 * gamma_e * X(15);
F(16) = gamma_g^2 * (N_pi * get_Qi - X(12)) - 2 * gamma_g * X(16);
F(17) = gamma_g^2 * (N_ii * get_Qi - X(13)) - 2 * gamma_g * X(17);
F(18) = (alpha_Na * get_Qp - Na_pump)/tau_Na;
F(19) = (g_KNa_0 * (2*X(5))*(1-0.6*X(4))*(1-0.95*X(6)) - X(19))/tau_g;
F(20) = (sigma_p_0 - (4*X(4) + 2*X(6)) - X(20))/tau_s;
